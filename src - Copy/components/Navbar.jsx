import { useEffect, useState } from 'react';
import { cn } from '../lib/utils.js';
import { X, Menu } from 'lucide-react';

const items = [
    { name: "Home", href: "#home" },
    { name: "About", href: "#about" },
    { name: "Skills", href: "#skills" },
    { name: "Projects", href: "#projects" },
    { name: "Contact", href: "#contact" },
]; 

export const Navbar = () => {
    const [hasScrolled, setHasScrolled] = useState(false);
    const [isMenuOpen, setIsMenuOpen] = useState(false);

    useEffect(() => {
        const handleScroll = () => {
            setHasScrolled(window.screenY > 10);
        };
        window.addEventListener("scroll", handleScroll);
        return () => window.removeEventListener("scroll", handleScroll);
    }, []);

    return <nav className={cn("fixed", "w-full", "z-40", 
        "transition-all", "duration-300",
        hasScrolled ? "py-3 bg-background/80 backdrop-blur-md shadow-xs" : "py-5")}>

            <div className={cn("container", "flex", "items-center", "justify-between")}>
                <a className={cn("text-xl", "font-bold", "text-primary", "flex", "items-center")} href="#home">
                    <span className={cn("relative", "z-10")}>
                        <span className={cn("text-glow", "text-foreground")}>Aryan Panchal</span> Portfolio
                    </span>
                </a>

                {/* desktop navbar */}
                <div className={cn("hidden", "md:flex", "space-x-8")}>
                    {items.map((items, key) => (
                        <a key={key} href={items.href}
                        className={cn("text-foreground/80", "hover:text-primary",
                                        "transition-colors", "duration-300")} >
                            {items.name}
                        </a>
                    ))}
                </div>

                {/* mobile navbar */}
                <button onClick={() => setIsMenuOpen(!isMenuOpen)}
                        className={cn("md:hidden", "p-2", "text-foreground", "z-50")}
                        aria-label={isMenuOpen ? "Close Menu" : "Open Menu"}>{
                            isMenuOpen ? <X size={24}/> : <Menu size={24}/>
                }</button>
                <div className={cn("fixed", "inset-0", "bg-background/95", 
                                    "backdrop-blur-md", "z-40", "flex", "flex-col", "items-center",
                                    "justify-center", "transition-all", "duration-300", "md:hidden",
                                    isMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none")}>
                    <div className={cn("flex", "flex-col", "space-y-8", "text-xl")}>
                        {items.map((items, key) => (
                            <a key={key} href={items.href}
                            className={cn("text-foreground/80", "hover:text-primary",
                                            "transition-colors", "duration-300")} >
                                {items.name}
                            </a>
                        ))}
                    </div>
                </div>
            </div>

    </nav>
};